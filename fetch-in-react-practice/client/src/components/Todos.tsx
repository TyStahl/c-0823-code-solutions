...
export function Todos() {
  ...
  async function addTodo(newTodo: UnsavedTodo) {
    try {
      ...
      if (!res.ok) throw new Error(`An error occured: ${res.status}`);
      const tdobj = await res.json();
      setTodos(todos.concat(tdobj));
    } catch (e) {
      console.error(e);
    }
  }
  ...
  async function toggleCompleted(todoId: number) {
    const found = await todos.find((element) => element.todoId === todoId);
    const isComplete = found?.isCompleted;
    const todoObj = { isCompleted: !isComplete };
    try {
